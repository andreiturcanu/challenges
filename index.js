...
function rmvCharAt(str, pos){
   return (pos < str.length+1) ? str.slice(0, pos-1) + str.slice(pos) : "Cannot do, position is longer than string";
}
console.log(rmvCharAt("Some", 4));
...
function chgPos(str){
   return str.length === 1 ? str : str.length > 1 ? str[str.length-1] + str.slice(1, str.length-1) + str[0] : "String too short";
}
...
function chkM3orM7(num){
   if (num > 0) {
      return Number.isInteger(num / 3) ?
          Number.isInteger(num / 7) ? "Multiple of both 3 and 7" : "Multiple of 3" :
          Number.isInteger(num / 7) ? "Multiple of 7" :
...
function addThree(str) {
   return str.length >= 3 ? str.substr(0, 3) + str + str.slice(str.length-3) : "String too short";
}
...
function strAt5th(str){
   return str.indexOf("Script") === 4 ? str.substr(0, 4) + str.slice("Script".length + 4) : str;
}
...
function largestIntOfThree(int1, int2, int3) {
   return Number.isInteger(int1 * int2 * int3) ? Math.max(int1, int2, int3) : "One of the numbers is not an integer";
}
...
function nearest100(int1, int2) {
   return Number.isInteger(int1 * int2) ?
       Math.abs(int1-100) > Math.abs(int2-100) ? int2 :
           Math.abs(int1-100) === Math.abs(int2-100) ?
               "Both numbers are equally distanced from 100" : int1
